...
async function sendVoiceOrTextReply(msg, text, options = {}) {
  const useVoice = config.tts.enabled && config.tts.autoReply && !options.forceText;
  if (!useVoice || text.length > config.tts.maxChars) {
    await msg.reply(text);
    return;
  }

  try {
    const voice = await services.generateVoice(text);
    if (voice.ok) {
      const media = new MessageMedia(voice.mimetype, voice.data, voice.filename);
      await msg.reply(media, null, { sendAudioAsVoice: true });
    } else {
      await msg.reply(text);
    }
  } catch (error) {
    logger.error("Erreur envoi vocal", { error: error.message });
    await msg.reply(text);
  }
}
...
function initializeClient() {
  ...
  client = new Client({
    ...
    puppeteer: {
      headless: true,
      // Sur Render, on utilise Chromium du système
      ...(isRender && { executablePath: "/usr/bin/chromium-browser" }),
      args: [
        "--no-sandbox",
  ...
  client.on("ready", async () => {
    ...
    logger.info(
      `⏰ Plage d'absence: ${config.bot.absenceStart} → ${config.bot.absenceEnd}`,
    );
    const botActive = services.isBotActive();
    logger.info(
      `🤖 Statut: ${botActive ? "🔴 ABSENT (bot actif)" : "🟢 PRÉSENT (bot inactif)"}`,
    );
    if (config.bot.demoMode)
      logger.warn("⚠️ MODE DÉMO ACTIF - le bot répond toujours");

    // Rattrapage: repondre aux anciens messages si le bot est actif
    if (botActive) {
      await services.catchUpPendingConversations(client, config.user.phone);
    }
  });
...
async function handleReconnect() {
  ...
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    logger.error(
      `❌ Reconnexion échouée après ${MAX_RECONNECT_ATTEMPTS} tentatives`,
    );
    shutdown('RECONNECT_FAILURE');
    return;
  }
  ...
  logger.warn(
    `🔄 Reconnexion tentative ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} dans ${delay}ms...`,
  );
  ...
  setTimeout(async () => {
    try {
      ...
      logger.error('Erreur lors de la reconnexion', { error: error.message });
    } finally {
...
async function handleMessage(msg) {
  ...
  if (msg.type === "notification_template" || msg.type === "e2e_notification")
    return;
  ...
  const incomingText = services.sanitizeMessage(
    mediaResult.text || msg.body || mediaContext || "[media recu]",
  );
  ...
  // ========== PROPRIÉTAIRE ==========
  if (isOwner) {
    // Commande email: !email destinataire | sujet | corps
    const emailMatch = incomingText.match(/^!email\s+(.+?)\s*[|]\s*(.+?)\s*[|]\s*([\s\S]+)/i);
    if (emailMatch) {
      const [, to, subject, body] = emailMatch;
      try {
        await msg.reply(`📧 Envoi d'email à ${to}...`);
        const info = await email.sendEmail(to.trim(), subject.trim(), body.trim());
        await msg.reply(`✅ Email envoyé à ${to.trim()}\n📌 Sujet: ${subject.trim()}\n🆔 ${info.messageId}`);
        await db.saveMessage(phone, "assistant", `Email envoyé à ${to}: ${subject}`);
        await db.logEmail('sent', to.trim(), null, subject.trim(), 'sent', info.messageId, null);
      } catch (error) {
        await msg.reply(`❌ Échec envoi email: ${error.message}`);
        await db.logEmail('sent', to.trim(), null, subject.trim() || '', 'error', null, error.message);
      }
      return;
    }
    ...
    try {
      ...
      await msg.reply(
        "⚠️ Erreur technique - impossible de générer une réponse.",
      );
    }
    return;
  }
  ...
  if (!services.isBotActive()) {
    ...
    logger.info(
      `📍 Statut: ${currentStatus.label} (${services.getCurrentHour()}h) - réponse du statut`,
    );
  ...
  try {
    // Générer la réponse
    const currentStatus = statuses.getStatus();
    const response = await services.generateAbsenceResponse(
      senderName,
      incomingText,
      history,
      {
    ...
    // Envoyer (vocal si TTS actif)
    await sendVoiceOrTextReply(msg, response);
    await db.saveMessage(phone, "assistant", response);

    // Si l'IA n'a pas pu répondre (fallback) ou message complexe, sauvegarder pour intervention
    if (response.includes("te répondra dès son retour") || response.includes("n'ai pas pu générer")) {
      await db.addToInterventionQueue(phone, senderName, incomingText, "Réponse IA non générée");
    }
    ...
    // Message de fallback professionnel + intervention
    ...
    try {
      await sendVoiceOrTextReply(msg, fallback);
      await db.addToInterventionQueue(phone, senderName, incomingText, "Erreur API / fallback");
    } catch (err) {
...
async function startup() {
  ...
  try {
    ...
    // Dashboard local (apres client pour pouvoir repondre)
    dashboard.setClient(client);
    dashboard.startDashboard();

    // Verifier toutes les 5 min si le bot vient de devenir actif
    let wasActive = services.isBotActive();
    setInterval(async () => {
      const nowActive = services.isBotActive();
      if (nowActive && !wasActive) {
        logger.info("🔄 Transition: bot devenu actif → rattrapage des messages");
        await services.catchUpPendingConversations(client, config.user.phone);
      }
      wasActive = nowActive;
    }, 5 * 60 * 1000);
